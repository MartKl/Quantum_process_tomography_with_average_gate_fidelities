...
% Delta(m)
m_list=[160:10:340 360:20:520 550:30:670  710 750:50:800 900 1000 1100 1200:200:1600 2000];
eta_list = [.1, .01, .001]; %measurement noise

% Delta(eta)
% m_list=[200, 300, 500, 800, 1200, 2000];
% eta_list = logspace(log10(.5),-4,50); %measurement noise
...
[w, x, y, z]= ndgrid(1:mL,1:numQL,1:etaL,1:reps);
jobs = horzcat(reshape(w,[anz,1]),reshape(x,[anz,1]),reshape(y,[anz,1]),reshape(z,[anz,1]));
jobs = [jobs(:,1) jobs(:,4) jobs(:,2) jobs(:,3)]; % in order to run over reps at last
